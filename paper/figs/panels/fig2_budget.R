...
PSNR_FACET <- "Fidelity of the restored output (dB)"
...
p <- ggplot(long, aes(x = steps, y = value)) +
  geom_hline(data = marks, aes(yintercept = y), linewidth = 0.5, colour = "grey25") +
  geom_line(linewidth = 0.45, colour = "grey40") +
  geom_point(size = 2.3) +
  geom_text(aes(label = label), vjust = -1.15, size = 2.6) +
  geom_text(data = marks, aes(x = 275, y = y, label = "degraded input, left alone"),
            vjust = -0.55, size = 2.6, colour = "grey25", inherit.aes = FALSE) +
