...
closer to the reference on nearly every crop. Two inference passes of the same
model on the same crops differ by at most \NoiseMax{} decibels per crop. The
record does not show the sampling seed varied between the passes, so two runs
cannot be read as an estimate of sampling variance; the figure bounds only how
far re-running the recorded inference moved a crop. Reporting
...
bound crop rows; the E-STRATA record supplies the camera counts and means. It
...
the census population, and the resulting sample is informative only about the
part of that pool that fits. The scored crop128 comparison is a different
population: a published protocol, not the remnant of the skip. This is the same
structure as any
missing-not-at-random problem~\cite{rubin1976inference} when it applies to the
census, with the difference that
...
a way that leaves no trace in the summary itself. Naming an evidence ID instead of
a path is a schema change, not a process change.
...
measures only, on the published RealSR crop128 protocol, and the perceptual measure
...
No claim is made about the model's performance on every split of the benchmark it
was released against. The crops measured here are that published crop128
protocol, not the memory-selected census of \CensusTotal{} inputs, and the full
comparison was never produced. Its recorded status is
...
them by \GapBicubicAbs{} decibels on average. Re-running the recorded inference
moved no crop by more than \NoiseMax{} decibels; because the record does not
show the sampling seed varied between the two passes, that number bounds only
what a re-run moved and is not an estimate of sampling variance.
...
scored \SubsetN{} crops are the published crop128 protocol. A separate census
dropped \CensusSkipped{} of \CensusTotal{} inputs by output size and left the
...
evidence ID in a manifest, and the figure code verifies the bound bytes before reading
them, so the build fails rather than silently
