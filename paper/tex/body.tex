...
A labelled check, not a re-run and not a ranking of other methods, places that
luma-with-shave mean \PublishedAlignDelta{} decibels from the published RealSR
cell \PublishedDitPsnr{} in the released model's own supplement; the later
local sample of the same protocol sits \PublishedAlignLocalDelta{} decibels
from the same cell. The published StableSR cell on that row is
\PublishedStableSR{}, \PublishedCollisionDelta{} decibels from the bound local
mean, and is disclosed so the two cells are not swapped. The published
structural-similarity cell on the same row is \PublishedDitSsim{},
\PublishedSsimDelta{} from the bound luma-with-shave structural mean. The
other ten tabulated methods were not re-run, and this alignment does not
rewrite the thesis.
...
removed them. A leftover stem-name check finds \CensusNameOverlap{} shared
names between the scored crop128 stems and that skip list, and
\CensusNameOverlapRan{} shared names with the outputs that ran; the names
belong to different size classes and are not joined.
...
dated directory, the predeclaration, nine-cell summary, independent
analysis and receipt of the initialisation ablation, the labelled
published-table alignment and its hashed supplement, and the leftover
census-versus-crop128 stem-name check accompany the manuscript
