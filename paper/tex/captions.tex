...
The left panel is the training objective at the last step; the right panel is
mean peak signal-to-noise ratio of the restored output against the reference
image, with the horizontal rule marking the same quantity for the degraded input
passed through unchanged. The objective falls by a factor of \ToyLossDrop{}. The
fidelity of the output falls with it, by \ToyLongRegression{} decibels, and stays
below the untouched input at both budgets. Read alone, either panel would give
the opposite impression of what the extra budget bought. The longer run was
...
\newcommand{\CapPaired}{%
The paired differences, and the width the model varies by against itself. The
left panel plots one point per crop: the peak signal-to-noise ratio of a released
...
run-to-run variation; the gap is \NoiseMargin{} times that width. The right panel
...
property of each crop and was fixed before these differences were computed.%
}
...
skipped before inference. The left panel gives, for that attempt and for the
...
decision. The right panel breaks the dropped inputs down by the source group
their file names record, and shades the two groups that appear in the later
subset. Every dropped input was of a single larger size class, so what survived
the ceiling was fixed by output dimensions rather than chosen.%
}
...
them. Groups are the prefixes the input file names carry, which correspond to the
capture device. The final column records whether that group appears in the
